	...
	tests := []struct {
		...
		},
		{
			...
			config: &Config{
				...
				MarshalerName: awss3exporter.OtlpProtobuf,
				APIURL:        "https://this-is-a-very-long-hostname-that-exceeds-the-maximum-allowed-length-for-a-hostname-which-is-253-characters-according-to-rfc-standards-and-should-therefore-fail-validation-when-we-test-it-in-our-configuration-validation-tests-to-ensure-that-our-hostname-validation-logic-is-working-correctly-and-properly-rejecting-hostnames-that-are-too-long",
				APIKey:        configopaque.String("test-key"),
				APISecret:     configopaque.String("test-secret"),
			},
			expectError: true,
